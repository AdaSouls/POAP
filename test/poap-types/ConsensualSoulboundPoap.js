...
async function deployPoapFixture() {
    ...
    const soulboundPoapFactory = await ethers.getContractFactory("ConsensualSoulboundPoap");
    const soulboundPoapToken = await soulboundPoapFactory.deploy("Test Consensual Soulbound Poap", "TCSPOAP", owner.address);
...
describe("Consensual Soulbound Poap Contract", function () {
    ...
    describe("Deployment", function () {
        ...
        it("Should assign the name correctly", async function () {
            ...
            expect(soulboundPoapName).to.equal("Test Consensual Soulbound Poap");
        });
        ...
        it("Should assign the symbol correctly", async function () {
            ...
            expect(soulboundPoapSymbol).to.equal("TCSPOAP");
        });
    ...
    describe("Functions", function () {
        ...
        describe("createEventId", function () {
            ...
            it("Should not allow the creation of events with same id", async function () {
                ...
                await expect(soulboundPoapToken.createEventId(1, 0, latestPlusSevenDays, addr1.address)).to.be.revertedWith("ConsensualSoulboundPoap: event already created");

                await expect(soulboundPoapToken.createEventId(2, 100, latestPlusSevenDays, addr2.address)).to.be.fulfilled;

                await expect(soulboundPoapToken.createEventId(2, 100, latestPlusSevenDays, addr2.address)).to.be.revertedWith("ConsensualSoulboundPoap: event already created");
            ...
            it("Should not allow the creation of events with mint expiration date less than 3 days from now", async function () {
                ...
                await expect(soulboundPoapToken.createEventId(2, 0, latest, addr2.address)).to.be.revertedWith("ConsensualSoulboundPoap: mint expiration must be higher than current timestamp plus 3 days");
        ...
        describe("issue", function () {
            ...
            it("Should only be callable by an event minter (admin or organizer)", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.be.fulfilled; // admin call

                await expect(soulboundPoapToken.connect(addr1).issue(1, addr2.address, "InitialState", true, 0)).to.be.fulfilled; // organizer call

                await expect(soulboundPoapToken.connect(addr2).issue(1, addr2.address, "InitialState", true, 0)).to.be.reverted; // not admin nor organizer call
            ...
            it("Should only be callable when contract is not paused", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.be.fulfilled;

                await soulboundPoapToken.pause();

                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.be.reverted;
            ...
            it("Should only mint tokens for previously created events (createEventId)", async function () {
                ...
                await expect(soulboundPoapToken.issue(2, addr2.address, "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: event does not exist");
            ...
            it("Should only mint tokens if mint expiration date is not met (or not set)", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr1.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issue(2, addr2.address, "InitialState", true, 0)).to.be.fulfilled;

                await time.increaseTo(latestPlusEightDays);

                await expect(soulboundPoapToken.issue(1, addr1.address, "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: event mint has expired");
                await expect(soulboundPoapToken.issue(2, addr2.address, "InitialState", true, 0)).to.be.fulfilled;
            ...
            it("Should only mint tokens if total supply < max supply", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: max supply reached for event");
            ...
            it("Should assign the right token id to the token", async function () {
                ...
                await soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0);
                await soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0);

                await expect(soulboundPoapToken.createEventId(2, 5, latestPlusSevenDays, addr1.address)).to.be.fulfilled;

                await soulboundPoapToken.issue(2, addr2.address, "InitialState", true, 0);
                await soulboundPoapToken.issue(2, addr2.address, "InitialState", true, 0);

                // TODO: check how to get the token id returned by issue function
            ...
            it("Should emit EventToken event", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.emit(soulboundPoapToken, "EventToken").withArgs(1, 1);

            });

            it("Should not emit Locked event if token not locked", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", false, 0)).to.not.emit(soulboundPoapToken, "Locked");

            });

            it("Should emit Locked event if token is locked", async function () {
                ...
                await expect(soulboundPoapToken.issue(1, addr2.address, "InitialState", true, 0)).to.emit(soulboundPoapToken, "Locked").withArgs(1);

            });

        })

        describe("issueEventToManyUsers", function () {
            ...
            it("Should only be callable by an event minter (admin or organizer)", async function () {
                ...
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled; // admin call

                await expect(soulboundPoapToken.connect(addr1).issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled; // organizer call

                await expect(soulboundPoapToken.connect(addr2).issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.reverted; // not admin nor organizer call
            ...
            it("Should only be callable when contract is not paused", async function () {
                ...
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled;

                await soulboundPoapToken.pause();

                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.reverted;
            ...
            it("Should only mint tokens for previously created events (createEventId)", async function () {
                ...
                await expect(soulboundPoapToken.issueEventToManyUsers(2, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: event does not exist");
            ...
            it("Should only mint tokens if mint expiration date is not met (or not set)", async function () {
                ...
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueEventToManyUsers(2, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled;

                await time.increaseTo(latestPlusEightDays);

                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: event mint has expired");
                await expect(soulboundPoapToken.issueEventToManyUsers(2, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled;
            ...
            it("Should only mint tokens if total supply < max supply", async function () {
                ...
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: max supply reached for event");
            ...
            it("Should assign the right token id to the token", async function () {
                ...
                await soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0);
                await soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0);

                await expect(soulboundPoapToken.createEventId(2, 6, latestPlusSevenDays, addr1.address)).to.be.fulfilled;

                await soulboundPoapToken.issueEventToManyUsers(2, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0);
                await soulboundPoapToken.issueEventToManyUsers(2, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0);

                // TODO: check how to get the token id returned by issueEventToManyUsers function
                expect(1).to.be.equal(1);
                expect(2).to.be.equal(2);
                expect(3).to.be.equal(3);
                expect(4).to.be.equal(4);
            ...
            it("Should emit EventToken event", async function () {
                ...
                await expect(soulboundPoapToken.issueEventToManyUsers(1, [addr2.address, addr3.address, addr4.address], "InitialState", true, 0))
        ...
        describe("issueUserToManyEvents", function () {
            ...
            it("Should only be callable by an admin", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0)).to.be.fulfilled; // admin call

                await expect(soulboundPoapToken.connect(addr1).issueUserToManyEvents([1, 2, 3], addr3.address, "InitialState", true, 0)).to.be.reverted; // organizer call

                await expect(soulboundPoapToken.connect(addr2).issueUserToManyEvents([1, 2, 3], addr4.address, "InitialState", true, 0)).to.be.reverted; // not admin nor organizer call
            ...
            it("Should only be callable when contract is not paused", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0)).to.be.fulfilled;

                await soulboundPoapToken.pause();

                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr3.address, "InitialState", true, 0)).to.be.reverted;

                await soulboundPoapToken.unpause();

                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr4.address, "InitialState", true, 0)).to.be.fulfilled;
            ...
            it("Should only mint tokens for previously created events (createEventId)", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3, 4], addr3.address, "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: event does not exist");
            ...
            it("Should only mint tokens if mint expiration date is not met (or not set)", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr3.address, "InitialState", true, 0)).to.be.fulfilled;

                await time.increaseTo(latestPlusEightDays);

                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr4.address, "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: event mint has expired");
                await expect(soulboundPoapToken.issueUserToManyEvents([2, 3], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
            ...
            it("Should only mint tokens if total supply < max supply", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr3.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr4.address, "InitialState", true, 0)).to.be.revertedWith("ConsensualSoulboundPoap: max supply reached for event");
            ...
            it("Should assign the right token id to the token", async function () {
                ...
                await soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0);
                await soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr3.address, "InitialState", true, 0);

                await soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr4.address, "InitialState", true, 0);
                await soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0);

                // TODO: check how to get the token id returned by issueUserToManyEvents function
                expect(1).to.be.equal(1);
                expect(2).to.be.equal(2);
                expect(3).to.be.equal(3);
                expect(4).to.be.equal(4);
            ...
            it("Should emit EventToken event", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr2.address, "InitialState", true, 0))
                    .to.emit(soulboundPoapToken, "EventToken").withArgs(1, 1)
        ...
        describe("transferFrom", function () {
            ...
            it("Should not allow the transfer of a locked token", async function () {
                ...
                await expect(soulboundPoapToken.connect(addr1).transferFrom(addr1.address, addr3.address, 1)).to.be.revertedWith("ConsensualSoulboundPoap: soulbound is locked to transfer");

            });

            it("Should allow the transfer of an unlocked token", async function () {
                ...
                await expect(soulboundPoapToken.createEventId(1, 0, latestPlusSevenDays, addr2.address)).to.be.fulfilled;
                await expect(soulboundPoapToken.issue(1, addr1.address, "InitialState", false, 0)).to.be.fulfilled;

                await expect(soulboundPoapToken.connect(addr1).transferFrom(addr1.address, addr3.address, 1)).to.be.fulfilled;
        ...
        describe("safeTransferFrom", function () {
            ...
            it("Should not allow the transfer of a locked token", async function () {
                ...
                await expect(soulboundPoapToken.connect(addr1).safeTransferFrom(addr1.address, addr3.address, 1)).to.be.revertedWith("ConsensualSoulboundPoap: soulbound is locked to transfer");
            ...
            it("Should allow the transfer of an unlocked token", async function () {
                ...
                await expect(soulboundPoapToken.connect(addr1).safeTransferFrom(addr1.address, addr3.address, 1)).to.be.fulfilled;
        ...
        describe("eventMaxSupply", function () {
            ...
            it("Should be callable by any address", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2, 3], addr1.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([2, 3], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
        ...
        describe("eventTotalSupply", function () {
            ...
            it("Should retrieve 0 as total supply for events that haven't yet minted any token (or don't exist)", async function () {
                ...
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2], addr1.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([1, 2], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issueUserToManyEvents([2], addr2.address, "InitialState", true, 0)).to.be.fulfilled;
        ...
        describe("burn", function () {
            ...
            it("Should decrease the total supply for the event", async function () {
                ...
                await expect(soulboundPoapToken.burn(2)).to.be.fulfilled;
            ...
            it("Should decrease the total supply for all poaps", async function () {
                ...
                await expect(soulboundPoapToken.createEventId(2, 0, latestPlusSevenDays, addr1.address)).to.be.fulfilled;
                await expect(soulboundPoapToken.issue(2, addr1.address, "InitialState", true, 0)).to.be.fulfilled;
                await expect(soulboundPoapToken.issue(2, addr1.address, "InitialState", true, 0)).to.be.fulfilled;
            ...
            it("Should emit Unlocked event", async function () {
                ...
                await expect(soulboundPoapToken.burn(1)).to.emit(soulboundPoapToken, "Unlocked").withArgs(1);
        ...
        describe("burnAuth", function () {
            ...
            /* it("Should only be callable by approved or owner and not by an admin", async function () {
                ...
                await expect(soulboundPoapToken.burn(1)).to.be.revertedWith("ConsensualSoulboundPoap: not authorized to burn");
